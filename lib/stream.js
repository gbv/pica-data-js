...
class PicaLineReader extends Transform {
  ...
  parseField(line, subfieldIndicator) {
    ...
    if (match[5].charAt(0) !== " ") {
      throw new LineError("Expected space character", subfieldPos)
    }
    if (match[5].charAt(1) != subfieldIndicator) {
      throw new LineError("Expected subfield indicator", subfieldPos + 1)
    }
  ...
  parsePlain(line) {
    ...
    for (let m of line.substr(pos).matchAll(/\$([^$])((?:[^$]+|\$\$)*)/g)) {
      ...
      if (!code.match(/^[A-Za-z0-9]$/)) {
        throw new LineError(`Invalid subfield code '${code}'`, pos + m.index + 2)
      }
  ...
  parseNormalized(line) {
    ...
    for (let i=0, fieldPos=0; i<fields.length; i++) {
      ...
      subfields.forEach(m => {
        ...
        if (!code.match(/^[A-Za-z0-9]$/)) {
          throw new LineError(`Invalid subfield code '${code}'`, pos + 2)
        }
    ...
    if (fields[fields.length-1] !== "") {
      throw new LineError("Expected field separator", line.length+1)
    }
    return record
  }
